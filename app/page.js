...
export default function Home(){
  ...
  return <main className="epoch-site">
    ...
    <section id="dynamic" data-main data-scroll-page className="main-page dynamic-page"><div className="page-sticky"><div className="dynamic-copy"><span className="eyebrow">PERSISTENT WORLD SIMULATION</span><h2>世界不会等待<br/>玩家上线。</h2><p>战斗、交易、运输、劫掠和占领会改变局部状态，并在短时间内制造新的价格差、资源点、任务需求和安全风险。</p><div className="change-list"><span>物价波动</span><span>任务刷新</span><span>护航需求</span><span>巡逻强度</span><span>冲突与繁荣</span><span>限时资源</span></div></div><div className="world-causality"><span className="causality-live"><i/>WORLD TICK / LIVE</span><div className="causality-chain">{['玩家行为','空间站状态','星系态势','星域变化','势力决策','风险 / 机遇窗口'].map((x,i)=><div key={x} style={{'--n':i}}><small>0{i+1}</small><b>{x}</b>{i<5&&<i className="fa-solid fa-arrow-down"/>}</div>)}</div><div className="event-window"><span>当前事件</span><b>贸易崩溃 · HELIOS 星域</b><p>资源涨价　护航任务增加　劫掠风险上升</p><small>窗口剩余 02:18:46</small></div><div className="intel-feeds"><span>星际星闻</span><span>酒吧谣言</span><span>航线变化</span></div></div></div></section>
