...
export default function Home(){
  ...
  return <main className="epoch-site">
    ...
    <section id="dynamic" data-main data-scroll-page className="main-page dynamic-page"><div className="page-sticky"><div className="dynamic-copy"><span className="eyebrow">PERSISTENT WORLD SIMULATION</span><h2>世界不会等待<br/>玩家上线。</h2><p>战斗、交易、运输、劫掠和占领会改变局部状态，并在短时间内制造新的价格差、资源点、任务需求和安全风险。</p><div className="change-list"><span>物价波动</span><span>任务刷新</span><span>护航需求</span><span>巡逻强度</span><span>冲突与繁荣</span><span>限时资源</span></div><div className="player-decision"><div><b>信息来源</b><span>星际星闻 · 酒吧谣言 · 交易与航线变化</span></div><div><b>可判断的机会</b><span>资源、护航、冲突升温与势力任务</span></div><div><b>行动选择</b><span>倒卖 · 采集 · 护航 · 封锁 · 争夺</span></div></div></div><div className="world-causality"><span className="causality-live"><i/>WORLD TICK / LIVE</span><div className="causality-chain">{['玩家行为','空间站上报','星系 / 星域上报','势力决策','事件下发','风险 / 机遇窗口'].map((x,i)=><div key={x} style={{'--n':i}}><small>0{i+1}</small><b>{x}</b>{i<5&&<i className="fa-solid fa-arrow-down"/>}</div>)}</div><div className="event-window"><span>持续攻击运输线路</span><div className="event-flow">{['物流与安全下降','库存减少 / 物价上涨','护航 · 倒卖 · 拦截','势力介入 / 冲突升级'].map((x,i)=><b key={x}>{String(i+1).padStart(2,'0')} {x}</b>)}</div><small>短期生成任务和价格波动 · 长期累积航线风险与战争机会</small></div><div className="intel-feeds"><span>局部战争</span><span>巡逻变化</span><span>护航生成</span><span>任务变更</span></div></div></div></section>
