...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    const update=()=>{const max=document.documentElement.scrollHeight-innerHeight,route=max?scrollY/max:0;root.style.setProperty('--route',route.toFixed(4));let best=0,dist=Infinity;els.forEach((el,i)=>{const r=el.getBoundingClientRect(),enter=Math.max(0,Math.min(1,(innerHeight-r.top)/(innerHeight*.55))),exit=Math.max(0,Math.min(1,r.bottom/(innerHeight*.55))),v=Math.min(enter,exit),p=enter<1?enter*.5:exit<1?1-exit*.5:.5;el.style.setProperty('--stage-p',p.toFixed(4));el.style.setProperty('--stage-v',v.toFixed(4));const d=Math.abs(r.top+r.height/2-innerHeight/2);if(d<dist){dist=d;best=i}});pages.forEach(el=>{const {p,v}=metrics(el);el.style.setProperty('--page-p',p.toFixed(4));el.style.setProperty('--page-v',v.toFixed(4))});setActive(x=>x===best?x:best);raf=0};
  ...
  return <main className="journey">
    ...
    {stages.map((s,i)=><Fragment key={s.id}><section id={s.id} className={`route-stage stage-${i}`}><div className="stage-sticky">
