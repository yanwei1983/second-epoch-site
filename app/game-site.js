...
import { languagePaths, languageStorageKey } from './site-language.mjs';
...
export default function GameSite({lang}){
  ...
  const content=<main className="epoch-site" lang={lang==='en'?'en':'zh-CN'}>
    ...
    <header className="epoch-nav">
      ...
      <label className="language-switch"><i className="fa-solid fa-globe" aria-hidden="true"/><span className="sr-only">选择语言</span><select value={lang} onChange={e=>{switchLanguage(e.target.value);e.currentTarget.blur()}} aria-label="选择语言"><option value="zh">中文</option><option value="en">English</option></select><i className="fa-solid fa-chevron-down" aria-hidden="true"/></label>
  ...
  return localizeNode(content,lang);
}
